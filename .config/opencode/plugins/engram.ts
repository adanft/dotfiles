/**
 * Engram — runtime plugin adapter
 *
 * Thin layer that connects the host event system to the Engram Go binary.
 * The Go binary runs as a local HTTP server and handles all persistence.
 *
 * Flow:
 *   Host events → this plugin → HTTP calls → engram serve → SQLite
 *
 * Session resilience:
 ...
 *   started (restart, reconnect, etc.). The session ID comes from host hooks
 *   (input.sessionID) rather than relying on a session.created event.
 */
...
const ENGRAM_BIN = process.env.ENGRAM_BIN ?? Bun.which("engram") ?? "/home/dioby/.local/bin/engram"
...
export const Engram: Plugin = async (ctx) => {
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      ...
      // input.sessionID comes from the host runtime — always available
      const sessionId = input.sessionID
      if (sessionId) {
        await ensureSession(sessionId)
