/**
 * background-agents
 * Unified delegation system for the configured runtime
 *
...
/**
 * Create a structured logger that sends messages to the host log API.
...
class DelegationManager {
  ...
  async delegate(input: DelegateInput): Promise<Delegation> {
    ...
    const { isSubAgent } = await parseAgentMode(this.client, input.parentAgent, this.log)
    const { isReadOnly } = await parseAgentWriteCapability(this.client, input.parentAgent, this.log)

    if (isSubAgent && !isReadOnly) {
      throw new Error(
        `Agent "${input.parentAgent}" cannot use delegate because it is a write-capable sub-agent. Use delegate from a coordinating/primary agent, or make the caller read-only.`,
      )
    }

    // Background delegations run in isolated sessions outside the host session tree.
    // The undo/branching system cannot track changes made in background sessions.
    // Delegate is therefore limited to coordinating agents and read-only sub-agents.
    ...
    // Agent param is critical for MCP tools - tells the host runtime which agent's config to use
...
Use \`delegation_read(id)\` to retrieve the full result. Results are persisted to disk and survive compaction.`,
    args: {
      ...
      agent: tool.schema
        .string()
        .describe(
          'Target sub-agent name, for example: "explore", "review", "research-analyst", "agent-creator", or "general".',
        ),
    },
...
agents in background sessions are NOT tracked by the host undo/branching system.
...
export const BackgroundAgents: Plugin = async (ctx) => {
  ...
  return {
    ...
    // `task` remains globally available, but `delegate` is hardened in execute() so
    // write-capable sub-agents cannot launch background sessions outside the tracked tree.
