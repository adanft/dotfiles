...
Read the `apply-progress` artifact if it exists and verify that TDD was actually followed. In `openspec`, this means `openspec/changes/{change-name}/apply-progress.md`:

```
Read apply-progress artifact:
├── If artifact missing:
│   ├── Flag: CRITICAL — Strict TDD evidence is missing
│   └── Continue the rest of verification; missing apply-progress is a finding, not a retrieval blocker
├── If artifact exists: find the "TDD Cycle Evidence" table
├── FOR EACH task row:
│   ├── RED column:
│   │   ├── Must say "✅ Written"
...
│   ├── GREEN column:
│   │   ├── Must say "✅ Passed"
│   │   ├── Cross-reference with Step 6b test execution results:
...
│
├── If artifact exists but NO "TDD Cycle Evidence" table found:
...
- ALWAYS check the TDD Cycle Evidence table from apply-progress when the artifact exists — it's the primary evidence source
