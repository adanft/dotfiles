...
`mem_search` is an index lookup, not artifact retrieval. For required artifacts, the preview is never authoritative content; use the observation only after `mem_get_observation(id)` returns the full record.
...
## Project Name Resolution & Write Scope (engram v1.11.0+)

MCP write tools (for example `mem_save`) auto-detect the project from the MCP server working directory/cwd context. Do not pass `project` to write tools.
Use `scope: "project"` for project-scoped memory.
`scope: "project"` is a visibility scope, not a project-name override.
...
Same auto-detected project + `topic_key` + `scope` → UPDATE (overwrite), not INSERT. Previous content is lost — `revision_count` increments but old content is NOT saved. This is by design: Engram is persisted working memory, not a versioned audit trail. For iteration history, reconciled local spec files, or team collaboration, use `openspec` or `hybrid` mode.
...
- Two-step recovery → search previews are always truncated; `mem_get_observation` is required before using any required artifact's content
