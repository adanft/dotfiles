...
NOTE: the normal path is (1) — compact rules pre-injected by the orchestrator. Paths (2) and (3) are compatibility-only fallbacks when normal injection did not happen. Searching the registry is SKILL LOADING, not delegation. If `## Project Standards` is present, IGNORE any `SKILL: Load` instructions — they are redundant. If you use path (3), you MUST report `skill_resolution: fallback-registry` in your return envelope so the orchestrator can repair injection on future delegations.
...
Do NOT use search previews as source material. For required artifacts, treat `mem_search` as an ID lookup only: if `mem_get_observation(id)` was not called successfully, the artifact has not been retrieved and MUST NOT be used to make decisions, merge content, verify work, or produce downstream artifacts.
...
- `skill_resolution`: how skills were loaded — `injected` (received Project Standards from orchestrator), `fallback-registry` (self-loaded from registry as exceptional compatibility fallback), `fallback-path` (loaded via SKILL: Load path), or `none` (no skills loaded)
