...
└── changes/                 <- Active changes
    ...
    └── {change-name}/       <- Active change folder
        ...
        ├── apply-progress.md <- from sdd-apply (cumulative progress + TDD evidence)
...
- `apply-progress.md` is cumulative. `sdd-apply` MUST merge prior progress when continuing a batch so completed work and TDD evidence survive across runs.
- `sdd-verify` SHOULD read `apply-progress.md` when present for execution scope, changed-file traceability, and Strict TDD evidence.
