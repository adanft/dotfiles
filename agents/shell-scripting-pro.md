...
2. Use `set -euo pipefail` for strict error mode when writing Bash-compatible scripts.
3. Quote variables properly to prevent word splitting and glob expansion.
4. Prefer built-in commands over external tools when possible.
5. Test scripts across different shell environments when feasible.
6. Document complex logic and provide usage examples.

## Output

- Robust shell scripts with proper error handling
- POSIX-compliant code for maximum compatibility when requested
- Comprehensive input validation and sanitization
- Clear usage documentation and help messages
- Modular functions for reusability
- Integration with logging and monitoring systems when relevant
